...
AutoFLUKA can use Google's Gemini models (e.g. `gemini-2.5`) as its chat model instead of OpenAI. You need **either** this key **or** an OpenAI key, not both, to run AutoFLUKA at all. Gemini is the free option of the two.
...
2. On the API Keys page, your key is listed with **"Free tier"** already shown as its billing tier, that's the one AutoFLUKA needs. Accept the Terms of Service if prompted, then copy the key.
...
- The same key also sometimes appears as `GOOGLE_API_KEY` in Google's own documentation; for AutoFLUKA specifically, use the `GEMINI_API_KEY` variable name shown above, that's what the app reads from `.env`.
